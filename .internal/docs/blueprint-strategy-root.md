...
The codebase (`code-base/budget-alerts-automation/`) is self-sufficient for deployment — `az deployment sub create` deploys all 9 modules. But the **subscription itself** must be prepared first. This document captures every platform-level setup that lives *outside* the Bicep templates — things that CI/CD pipelines, SHS admins, or future tenant rollouts must handle.
...
- Handing over to SHS FinOps Academy for production rollout
...
**This requires Owner or User Access Administrator** — which the deploying CSA typically does not have. This is a handoff item for the SHS admin.

```bash
# SHS admin runs this after deployment:
...
Many SHS subscriptions enforce tags via Azure Policy (deny effect). Deployment will fail with `RequestDisallowedByPolicy` if required tags are missing.

### Required Tags (SHS Standard)
...
| `managed-by` | No | — | `microsoft-csa` |
| `Purpose` | No | — | `MVP-validation-before-SHS-handover` |
...
| RBAC | Logic App MI needs Cost Mgmt Contributor | Auto-budget can't create budgets | SHS admin runs 1 CLI command post-deploy | **Handoff item** |
...
When SHS takes this to Azure DevOps or GitHub Actions:
...
When the MVP is no longer needed (after SHS promotes to staging/prod):
